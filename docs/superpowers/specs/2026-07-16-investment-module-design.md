...
后续若要增加全仓、双向持仓、币本位、USDC 本位或交割合约，应作为新的范围变更，
不能在 V1 实施中隐式扩展。
...
以下内容不改变产品范围，但属于实现正确性的强制约束；若后文章节仍使用简写，以本节
为准：

1. 日 K、分钟 K 和资金费率使用同表 SCD2 revision。当前版本用
   `revision_slot = 0` 表示，历史版本用自身 `revision` 作为 slot，并保存
   `valid_from/valid_to`。任何修订都必须保留旧值，确保 `data_as_of` 可重放。
2. `investment_job` 使用短事务 claim、租约、heartbeat 和 fencing token。HTTP、LLM、
   报告生成和回测不得在 claim 事务中执行；完成或失败更新必须匹配 claim token。
3. Investment Agent 的只读工具按单次运行创建并在服务端绑定 workspace、account、actor
   和 `data_as_of`，不得注册进通用聊天的全局工具集合。LLM 运行期间不暴露任何有交易
   副作用的工具；最终 decision 校验并持久化后，Runner 才能进入唯一的交易执行路径。
   崩溃重试可以用同一幂等键再次进入 Facade，但最终只能存在一个 intent、order 和资金效果。
4. 模拟账户创建时必须显式提交带单位的风险限制；金额、名义价值、比例和 bps 字段使用
   清晰后缀。账户创建后 `trading_enabled` 和 `agent_auto_trade_enabled` 默认均为
   `false`，不使用未经确认的隐式额度。
5. 回测请求必须拒绝未知字段。不能依赖全局 Jackson 的忽略未知字段设置，否则前端仍可
   偷带策略参数。
6. 下一根已关闭 1 分钟 K 线尚未出现时，风控通过只产生 `PENDING` 模拟订单和撮合任务；
   Agent 只能得到 `ACCEPTED/PENDING_MATCH`，不能报告已成交。
7. 生产数据订阅注册表和生产策略注册表初始为空。测试 fixture 可以提供虚拟 Provider 和
   测试策略；实际 symbol 与私人策略必须在后续 Java 代码提交中显式加入。
...
未来 Bitget 适配阶段的实现示例（V1 不创建该生产 Bean）：

```text
BitgetUsdtFuturesSubscriptionProvider
```

未来该类使用 `List.of(...)` 或其他明确代码结构返回允许接入的交易对和数据能力。V1
生产注册表返回空集合，只有 `src/test` fixture 注册虚拟订阅；后续变更订阅范围需要改
代码、测试、审核和发布。
...
- worker 在短事务中使用 `FOR UPDATE SKIP LOCKED` 抢占并提交，之后才执行任务。
- 任务带 `idempotency_key`。
- 失败按错误类型和指数退避重试。
- 超过最大重试次数进入 `FAILED`。
- worker 使用 `claim_token`、租约和 heartbeat；崩溃后租约可以被回收。
- complete、retry 和 fail 必须匹配 `id + locked_by + claim_token`，防止旧 worker 覆盖
  租约回收后的新结果。
- `defer(next_available_at)` 是正常等待状态，不增加失败 attempts；N+1 K 线未关闭和 GTC
  限价未穿越都使用 defer，而不是 retry/fail。
- 任何任务输入必须再次通过代码订阅策略校验。

内存定时器只负责创建幂等任务，不能直接承载任务状态。市场 planner 把代码订阅 schedule
展开为同步任务；生产注册表为空或 planner disabled 时严格 no-op。行情提交后事件与定时
reconciliation 共同产生/唤醒 PAPER_MATCH、PAPER_FUNDING_SETTLE 和 PAPER_MARGIN_CHECK，
保证正常实时路径与进程重启补偿都不会漏任务。

### 12.3 入库规则

- K 线使用批量 JDBC 写入；checksum 相同不写，发生修订时先关闭旧 revision，再插入
  新 current revision。不能用 `ON CONFLICT DO UPDATE` 原地覆盖旧事实。
- 单批建议 500 至 2000 行，实施时通过基准测试确认。
- 同一主键内容不变时不增加 revision。
- 已关闭 K 线被上游修订时保存旧值、增加 revision 并创建质量记录。
...
`dataset_snapshot` 是逻辑快照。V1 通过 current + SCD2 历史 revision 按
`data_as_of` 重放 PostgreSQL 数据，不必立即复制全部 K 线。规格和仓位档位使用规范化
JSON 完整复制并计算 SHA-256。未来 Python Worker 可以把快照导出到 Parquet。

在没有 Parquet artifact 时，仍被可重跑 snapshot 引用的 1 分钟数据不能物理删除；保留
任务应跳过对应范围。只有 artifact 已验证，或 snapshot 被显式标记为
`EXPIRED/NOT_REPRODUCIBLE` 后，才能清理该范围。
...
GTC 订单在某根 K 线未穿越时继续等待后续已关闭 K 线，不消耗失败次数。只有用户取消、
明确过期、风险状态使其失效或实际成交后才进入终态。市价单的 N+1 K 线尚未关闭时同样
等待，不能用当前未关闭 K 线成交。
...
LLM 运行期间不提供写入工具。上述只读工具不能作为全局 `ToolCallback` Bean 暴露给
普通聊天；每次 Investment Agent 运行动态创建 scoped callbacks，并由服务端闭包绑定
`workspace_id`、`account_id`、`actor_id` 和 `data_as_of`。LLM 参数中不接受这些安全
边界字段。

`submit_paper_trade_intent` 是 Runner 内部的执行步骤，不是 LLM ToolCallback。只有完整
结构化 decision 通过 Schema、权限、订阅和 `data_as_of` 校验，并先持久化审计记录后，
Runner 才能进入该 decision 唯一的执行路径。执行服务用
`run + decision + instrument` 幂等键调用 `PaperTradingFacade.submitIntent(...)`；如果进程在
Facade 已提交而 decision 尚未链接 intent 时崩溃，重试可以携带同一幂等键再次调用并读取
同一个 intent。物理调用次数不作为契约，唯一 intent、order 和资金效果才是契约。HOLD、
超时、非法 JSON、未知标的或校验失败均为零交易副作用。
...
sequenceDiagram
    ...
    J->>A: read-only tools + context with data_as_of
    A-->>J: structured decision proposal
    J->>J: validate and persist decision audit
    J->>F: idempotent submit after validation
    ...
    alt risk rejected
        ...
        M-->>F: create pending order and match job
        F-->>J: accepted/pending match
        Note over M,L: N+1 closed 1m bar becomes available
        M->>L: fill and apply transaction
    end
```

不出现人工确认状态。账户开关打开且风险全部通过后，Agent 订单自动进入撮合。撮合是
异步状态；只有撮合任务实际写入 fill、ledger 和 position 后才可报告已成交。
AUTO_TRADE 运行必须禁用并行工具执行。模型重试、重复 proposal 或任务重跑只能命中同一
decision/intent 幂等事实；允许用相同幂等键重入 Facade 完成崩溃恢复，但不能产生第二个
intent、order、fill、ledger 或 position 效果。
...
- 同一 workspace/account、Agent preset、run type、规范化 instrument 集合和 `data_as_of`
  只能有一个有效运行；instrument id 排序去重后参与 input hash。
...
V1 通用 migration 必须同时兼容 H2 测试和 PostgreSQL，因此软删除业务键使用普通唯一
约束，不使用部分唯一索引，也不使用 `(business_key, deleted)`。删除后如需同名对象，
走恢复/重新激活语义。分区、BRIN 和部分索引只在有真实规模证据后通过新的 PostgreSQL
versioned migration 增加。
...
- unique `(venue_id, product_type, symbol)`
...
- `ingested_at`
- `revision`
- `revision_slot`：current 为 0，历史行使用自身 revision
- `valid_from`
- `valid_to` nullable
- `checksum`
...
- `(instrument_id, price_type, revision_slot, bar_date desc)`
...
- `(source_id, instrument_id, funding_time, revision)`
...
- `(instrument_id, revision_slot, funding_time desc)`
...
`price_type` 和 `interval_code` 不允许为 null；不适用时使用 `NONE` sentinel，避免
PostgreSQL nullable 复合唯一键绕过幂等约束。
...
`price_type` 和 `interval_code` 不允许为 null；不适用时使用 `NONE` sentinel。
...
创建账户时两个交易开关强制为 false，并在同一事务创建显式风险配置；不能使用未确认的
隐藏额度默认值。
...
- `limit_price`
- `intent_id` nullable
- `execution_status`：NOT_APPLICABLE、PENDING、SUBMITTED、FAILED
- `execution_idempotency_key` nullable
- `data_as_of`
- `expires_at`
- `status`
- `created_at`
...
- `(instrument_id, data_as_of desc)`
- unique `(intent_id)`
- unique `(execution_idempotency_key)`

HOLD 使用 `NOT_APPLICABLE` 且没有 execution key。通过校验的非 HOLD decision 在调用 Facade
前保存 `PENDING + execution_idempotency_key`；链接 intent 后转 `SUBMITTED`。重启时扫描仍为
PENDING 且未链接 intent 的 decision，并用原 key 重入 Facade 恢复。
...
- 不为每个价格或指标字段创建索引。
- 任务队列初始使用普通 `(status, available_at, priority, id)` 索引；有真实 PostgreSQL
  规模和查询计划证据后，才通过新的 versioned migration 增加 Pending 部分索引。
...
Investment API 中价格、数量、金额、名义价值、比率和指标统一序列化为十进制字符串；
前端只在图表绘制边界转换为 JavaScript `number`。列表统一使用现有分页 envelope。K 线
必须按时间升序返回，并带 `isClosed`、`revision`、`dataAsOf` 和明确的最大点数限制。
...
GET            /api/investment/workspaces/{workspaceId}/overview
...
Overview 是 workspace-scoped 聚合接口，`workspaceId` 必填。尚未创建或选择 workspace 时，
Overview 页面只展示创建/选择空态；公共行情仍可直接从 Market 页面访问。

报告 POST 创建 `PENDING` 报告和幂等 `REPORT_BUILD` job，并立即返回排队状态；详情响应
包含 evidence 列表，不要求前端再按 evidence 逐条查询。
...
GET  /api/investment/backtests/{runId}/events
GET  /api/investment/backtests/{runId}/equity
```

不提供策略创建、修改、删除和参数保存 API。
回测 POST 请求只接受 spec 15.1 的字段，并显式拒绝所有未知字段。
...
GET       /api/investment/paper-accounts/{accountId}/fills?instrumentId=&from=&to=&page=&size=
...
`POST trade-intents` 返回 intent、逐条风险结果以及当前 order/fill 摘要；下一根 K 线尚未
可用时明确返回 `PENDING_MATCH`。

fills 查询必须 owner-scoped，按 `event_time, id` 稳定排序，并限制时间跨度和分页大小；
K 线标记请求必须提交 `instrumentId/from/to`。供 K 线标记使用的 DTO 至少包含
`instrument_id`、`market_bar_open_time`、`event_time`、
`side`、`action_type`、`order_origin`、`event_type`、`price`、`quantity` 和
`liquidation`；`event_type` 明确区分普通成交和强平成交。
...
意图必须被风控拒绝。
Agent run 详情和 decision 响应必须包含 intent、risk、order、fill 的执行摘要，前端仅对
非终态 run 做有限轮询，不增加 WebSocket。
...
- 必须先选择或创建 workspace；无 workspace 时展示选择/创建空态，不调用 Overview API。
- 最新行情摘要。
- 私人模拟账户权益。
- 当前仓位和风险状态。
- 最近回测和 Agent 运行。
...
行情和合约输入先在事务外读取成不可变快照。接受事务随后：

1. 锁定 account。
2. 按 `instrument_id` 锁定相关 position，并锁定该账户所有会占用保证金的 pending open
   orders。
3. 重新读取余额、仓位、pending reserved margin、订单频率和 cooldown 状态。
4. 校验行情快照仍满足 revision 和 freshness 要求，并执行全部确定性风控。
5. 原子写入 intent、逐条 risk result；仅在全部通过时写唯一 pending order 和 match job。

两个并发意图合计超出额度时，只允许一个进入 `PENDING_MATCH`，另一个必须保存为
`RISK_REJECTED`，且不能产生第二张 order/job。
...
1. 锁定 account。
2. 按 `instrument_id` 排序锁定或创建 position。
3. 按 id 排序锁定 order。
4. 再次检查订单状态和保证金。
5. 写 fill。
6. 写 margin ledger。
7. 更新 wallet balance 和账户 `ledger_sequence`。
8. 更新 position。
9. 更新 order 状态。
10. 必要时写 account snapshot。

任何步骤失败全部回滚。

所有资金变化路径都必须遵循 `account -> positions -> orders -> append facts` 的相同锁序。
LLM、行情读取、指标和回测不允许在该事务中执行。
...
- Agent：workspace/account、preset、run type、排序去重的 instrument 集合 hash、data_as_of。
...
应用日志不得记录 prompt、messages、model response、tool arguments/results、私人报告、仓位或账户
快照全文；只保留类型、状态、耗时、长度和 hash 等摘要。受权限控制的数据库审计可以保存实现
追溯所需的结构化内容。不得记录任何未来可能加入的真实交易密钥。
...
实现前必须重新检查版本。数据库变更按四个可独立上线阶段拆分，每个阶段作为一次数据库
变更，只新增一个 versioned migration：

1. 行情、工作区和报告基础表。
2. 量化回测表。
3. 模拟账户和交易表。
4. Investment Agent 领域表。

四个 migration 必须串行创建和提交。每次开始前重新扫描最新版本；任何其他任务不得修改
已经提交的 Investment migration。
...
- 至少一个只存在于 `src/test` 的固定代码策略作为端到端 fixture；生产 Spring Context
  不注册样例或私人策略。
- 数据快照。
- Java 回测引擎。
- 合约手续费、资金费、杠杆和逐仓强平。
...
- 结构化决策 Schema。
- Runner 在完整校验和审计后内部执行 `submit_paper_trade_intent`，它不暴露为 LLM 工具。
- Agent 自动风险检查和模拟成交。
- Agent 运行、决策、报告和审计页面。
- 并发、幂等和自动交易开关验证。
...
V1 已确认按 `USDT-FUTURES + PERPETUAL + ISOLATED + ONE_WAY` 实施。若后续需要全仓、
双向持仓、币本位、USDC 本位或交割合约，应单独确认并修订对应范围和验收标准。
