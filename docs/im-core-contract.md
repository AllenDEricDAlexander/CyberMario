...
Channels and groups each expose a separate immutable `joinKey`. It is not the
database primary key and is not the context-scoped `channelKey`/`groupKey`.
New keys are generated by the system with disjoint `chn_` and `grp_`
namespaces; existing rows are backfilled by `V49__add_im_surface_join_keys.sql`.
The database enforces non-null values, type-specific prefixes, and unique
indexes. JPA maps the field as non-updatable and the entity permits assignment
only once.

Browser join requests send only `joinKey` to `POST /api/im/join-requests`.
`ImSurfaceJoinKeyService` resolves it through a cache-aside lookup: Guava is L1,
Redis is L2, and the database unique index is the fallback/source of truth.
Only the immutable key-to-id mapping is cached; membership mutation still
locks and revalidates the current surface row in the database.
...
- `POST/GET /api/im/platform/groups` creates an approval-based standalone
...
- Platform channels and groups are not publicly searchable. A user joins by
  entering the exact system-generated `joinKey`; `OPEN` activates immediately
  and `APPROVAL` creates an owner/admin review request. Direct owner/admin
  invitations remain supported as a second entry path.
...
- Platform standalone groups are joined by their exact immutable key or by a
  direct invitation. Platform child groups are visible only to active
  parent-channel members; their `OPEN`/`APPROVAL` policy applies only after
  that parent membership check.
...
|---|---|---|
| `PENDING` | `RoomFacade.applyJoinByKey` on an `APPROVAL` surface | Request exists and awaits owner/admin decision. |
...
The browser endpoint accepts only the immutable join key and never accepts a
surface type/database id pair. The trusted in-process `RoomFacade.applyJoin`
command remains for adapters such as Clocktower, but Platform channel and
standalone-group ID joins are rejected with `IM_PLATFORM_JOIN_KEY_REQUIRED`.
A Platform child-group application or approval additionally rechecks active
parent-channel membership.
...
- The frontend Clocktower chat panel also calls generic IM REST endpoints for
  listing channels/groups/conversations/messages, sending, join-key joining,
  leaving, marking read, and using `/ws/im` for realtime.
...
- Create a public channel, copy its `chn_` key, join with that key, and verify
  the request never sends the database id.
- Create a group under that channel, join with its `grp_` key, and verify only active
...
- `IM_SURFACE_JOIN_KEY_REQUIRED` / `IM_SURFACE_JOIN_KEY_INVALID`: the browser
  join request omitted the generated key or supplied a malformed key.
- `IM_PLATFORM_JOIN_KEY_REQUIRED`: a Platform channel or standalone group was
  addressed by an internal database id instead of its generated join key.
...
- No channel at first login is expected. Create one, enter a shared `chn_` key,
  or accept a direct channel invitation.
